...
On Linux systems without `root` or `CAP_NET_RAW`, ZMap can now fall back to an unprivileged backend for compatible probe modules. You can force that path with `--unprivileged`; otherwise ZMap will enable it automatically when raw sockets are unavailable. This is intended for non-root environments such as Termux.
...
For user-local source builds on Termux or non-root Linux, use `sh scripts/setup-local.sh`. It validates the environment, builds, and installs without assuming `sudo` or `apt`. On Termux it automatically uses `$PREFIX` as the dependency prefix, so installed packages are discovered without extra exports.
